...
const LM_STUDIO_API_URL = (process.env.LM_STUDIO_API_URL || "http://localhost:1234").replace(/\/$/, "");
const LM_STUDIO_API_KEY = process.env.LM_STUDIO_API_KEY || "";
const LM_STUDIO_DEFAULT_MODEL = process.env.LM_STUDIO_DEFAULT_MODEL || "qwen2.5-coder:14b";
...
You are powered by Claude Opus — use your advanced reasoning for planning, coordination, and synthesis.
Delegate resource-intensive sub-tasks to local GPU agents to save resources.

You have tools to build and manage your team autonomously:

- **spawn_agent**: Create a new specialist agent with a name, role, instructions, settings (wipe/continuity/boundaries), model, and toolTier.
  - Use \`model: "lmstudio/${LM_STUDIO_DEFAULT_MODEL}"\` for local GPU tasks (code search, file reading, simple generation).
  - Use \`model: "anthropic/${ANTHROPIC_MODEL}"\` only when a sub-agent needs advanced reasoning (rare).
  - Set \`toolTier: "basic"\` for research/read-only agents (can read files, search, grep, fetch web).
  - Set \`toolTier: "full"\` for builder agents (can also write files, patch code, run commands, git operations).
  - Set \`toolTier: "none"\` for pure chat/reasoning agents (no tools, just text responses).
...
2. spawn_agent for each specialist — typically a Research agent (basic tools, local model) and a Builder agent (full tools, local model).
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          model: { type: "string", description: "Model to use. Use 'lmstudio/<model>' for local GPU, 'anthropic/<model>' for cloud. Default: local GPU model." },
          toolTier: { type: "string", enum: ["none", "basic", "full"], description: "Tool access level. 'none'=chat only, 'basic'=read files/search/grep/web, 'full'=basic + write/exec/git. Default: 'none'." },
        },
      },
    },
  },
...
// ---------------------------------------------------------------------------
// Model backend routing — resolves "anthropic/...", "lmstudio/...", etc.
// ---------------------------------------------------------------------------

/** Determine which backend a model ID should route to. */
function resolveModelBackend(modelId) {
  const id = typeof modelId === "string" ? modelId.trim() : "";
  if (id.startsWith("anthropic/"))  return { backend: "anthropic",  model: id.slice("anthropic/".length) || ANTHROPIC_MODEL };
  if (id.startsWith("lmstudio/"))   return { backend: "lmstudio",   model: id.slice("lmstudio/".length) || LM_STUDIO_DEFAULT_MODEL };
  if (id.startsWith("local/"))      return { backend: "lmstudio",   model: id.slice("local/".length) || LM_STUDIO_DEFAULT_MODEL };
  if (id.startsWith("ollama/"))     return { backend: "lmstudio",   model: id.slice("ollama/".length) || LM_STUDIO_DEFAULT_MODEL };
  // If the default HERMES_MODEL starts with "anthropic/", route there
  if (HERMES_MODEL.startsWith("anthropic/")) {
    return { backend: "anthropic", model: id || ANTHROPIC_MODEL };
  }
  return { backend: "hermes", model: id || HERMES_MODEL };
}
...
function anthropicPost(apiPath, body) {
  return new Promise((resolve, reject) => {
    const urlStr = ANTHROPIC_API_URL + apiPath;
    let url;
    try { url = new URL(urlStr); } catch { reject(new Error(`Invalid URL: ${urlStr}`)); return; }
    const transport = url.protocol === "https:" ? https : http;
    const bodyStr = JSON.stringify(body);
...
function translateToAnthropic(messages, tools) {
  let systemPrompt = "";
    ...
    if (msg.role === "assistant" && msg.tool_calls && msg.tool_calls.length > 0) {
      ...
      for (const tc of msg.tool_calls) {
        ...
        if (typeof tc.function?.arguments === "string") {
          try { inputObj = JSON.parse(tc.function.arguments); } catch { inputObj = { _raw: tc.function.arguments }; }
  ...
  return { systemPrompt, messages: anthropicMessages, tools: anthropicTools };
}

/** Translate Anthropic response to OpenAI-compatible format. */
function translateFromAnthropic(response) {
  ...
  return { textContent, toolCalls, finishReason, stopReason };
}

/** Complete one turn via Anthropic Messages API (non-streaming). */
async function anthropicCompleteOneTurn(messages, model, tools) {
  const { systemPrompt, messages: anthropicMessages, tools: anthropicTools } = translateToAnthropic(messages, tools);
  ...
  const res = await anthropicPost("/v1/messages", body);
  const payload = await readJsonBody(res);

  if (res.statusCode >= 400) {
    const errMsg = payload?.error?.message || `Anthropic API HTTP ${res.statusCode}`;
    throw new Error(errMsg);
  }

  return { ...translateFromAnthropic(payload), resolvedModel: model || ANTHROPIC_MODEL };
}

/** Stream one turn via Anthropic Messages API (SSE). */
async function anthropicStreamOneTurn(messages, model, tools, onTextDelta, abortCheck) {
  const { systemPrompt, messages: anthropicMessages, tools: anthropicTools } = translateToAnthropic(messages, tools);
  ...
  if (res.statusCode >= 400) {
    res.resume();
    throw new Error(`Anthropic API HTTP ${res.statusCode}`);
  }
        ...
        try {
          ...
          if (eventType === "content_block_start") {
            ...
            if (block?.type === "tool_use") {
              toolBlocks[data.index] = { id: block.id, name: block.name, inputJson: "" };
            }
  ...
  const toolCalls = Object.values(toolBlocks).map((block) => {
    let args = {};
    try { args = JSON.parse(block.inputJson); } catch { args = { _raw: block.inputJson }; }
...
// ---------------------------------------------------------------------------
// LM Studio API client (OpenAI-compatible, different endpoint URL)
// ---------------------------------------------------------------------------

function lmStudioPost(apiPath, body) {
  return new Promise((resolve, reject) => {
    const urlStr = LM_STUDIO_API_URL + apiPath;
    let url;
    try { url = new URL(urlStr); } catch { reject(new Error(`Invalid URL: ${urlStr}`)); return; }
    const transport = url.protocol === "https:" ? https : http;
    const bodyStr = JSON.stringify(body);
...
let cachedLmStudioModels = null;
let cachedLmStudioModelsAt = 0;

async function fetchLmStudioModels() {
  const now = Date.now();
  if (cachedLmStudioModels && now - cachedLmStudioModelsAt < 30_000) return cachedLmStudioModels;
  try {
    const res = await lmStudioGet("/v1/models");
    if (res.statusCode >= 400) { res.resume(); return []; }
    const payload = await readJsonBody(res);
    const models = Array.isArray(payload?.data)
      ? payload.data.map((e) => (typeof e?.id === "string" ? e.id.trim() : "")).filter(Boolean)
      : [];
    cachedLmStudioModels = models;
    cachedLmStudioModelsAt = now;
    return models;
  } catch { return []; }
}

async function resolveLmStudioModel(requestedModel) {
  const trimmed = typeof requestedModel === "string" ? requestedModel.trim() : "";
  try {
    const models = await fetchLmStudioModels();
    if (models.length === 0) return trimmed || LM_STUDIO_DEFAULT_MODEL;
    if (models.includes(trimmed)) return trimmed;
    const byEnd = models.find((m) => m.endsWith(trimmed) || m.endsWith(`/${trimmed}`));
    if (byEnd) return byEnd;
    return models[0];
  } catch { return trimmed || LM_STUDIO_DEFAULT_MODEL; }
}

async function lmStudioCompleteOneTurn(messages, model, tools) {
  const resolvedModel = await resolveLmStudioModel(model);
  const body = { model: resolvedModel, messages, stream: false };
  if (tools && tools.length > 0) { body.tools = tools; body.tool_choice = "auto"; }

  const res = await lmStudioPost("/v1/chat/completions", body);
  const payload = await readJsonBody(res);
  if (res.statusCode >= 400) {
    throw new Error(extractOpenAiStyleError(payload, `LM Studio API HTTP ${res.statusCode}`));
  }
  const choice = Array.isArray(payload?.choices) ? payload.choices[0] : null;
  const message = choice?.message || {};
  const textContent = typeof message?.content === "string" ? message.content
    : Array.isArray(message?.content) ? message.content.map((p) => (typeof p?.text === "string" ? p.text : "")).join("") : "";
  const finishReason = typeof choice?.finish_reason === "string" && choice.finish_reason ? choice.finish_reason : "stop";
  const toolCalls = Array.isArray(message?.tool_calls)
    ? message.tool_calls.map((tc) => {
        let args = {};
        const rawArgs = tc?.function?.arguments;
        if (typeof rawArgs === "string" && rawArgs.trim()) { try { args = JSON.parse(rawArgs); } catch { args = { _raw: rawArgs }; } }
        return { id: typeof tc?.id === "string" ? tc.id : randomId(), name: typeof tc?.function?.name === "string" ? tc.function.name : "", args };
      })
    : [];
  return { textContent, toolCalls, finishReason, resolvedModel };
}

async function lmStudioStreamOneTurn(messages, model, tools, onTextDelta, abortCheck) {
  const resolvedModel = await resolveLmStudioModel(model);
  const body = { model: resolvedModel, messages, stream: true };
  if (tools && tools.length > 0) { body.tools = tools; body.tool_choice = "auto"; }

  const res = await lmStudioPost("/v1/chat/completions", body);
  if (res.statusCode >= 400) { res.resume(); throw new Error(`LM Studio API HTTP ${res.statusCode}`); }
    ...
    res.on("data", (chunk) => {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (typeof delta.content === "string" && delta.content) { textContent += delta.content; if (onTextDelta) onTextDelta(textContent); }
  ...
  const toolCalls = Object.values(toolCallAccum).map((tc) => {
    let args = {}; try { args = JSON.parse(tc.argsStr); } catch { args = { _raw: tc.argsStr }; }
    return { id: tc.id, name: tc.name, args };
  });

  if (!textContent.trim() && toolCalls.length === 0 && finishReason === "stop") {
    return lmStudioCompleteOneTurn(messages, resolvedModel, tools);
  }
  return { textContent, toolCalls, finishReason };
}

// ---------------------------------------------------------------------------
// Unified model routing — dispatches to correct backend
// ---------------------------------------------------------------------------

async function routedCompleteOneTurn(messages, model, tools) {
  const { backend, model: resolvedId } = resolveModelBackend(model);
  switch (backend) {
    case "anthropic":
      console.log(`[hermes-adapter] Routing to Anthropic: ${resolvedId}`);
      return anthropicCompleteOneTurn(messages, resolvedId, tools);
    case "lmstudio":
      console.log(`[hermes-adapter] Routing to LM Studio: ${resolvedId}`);
      return lmStudioCompleteOneTurn(messages, resolvedId, tools);
    default:
      return completeOneTurn(messages, model, tools);
  }
}

async function routedStreamOneTurn(messages, model, tools, onTextDelta, abortCheck) {
  const { backend, model: resolvedId } = resolveModelBackend(model);
  switch (backend) {
    case "anthropic":
      console.log(`[hermes-adapter] Streaming via Anthropic: ${resolvedId}`);
      return anthropicStreamOneTurn(messages, resolvedId, tools, onTextDelta, abortCheck);
    case "lmstudio":
      console.log(`[hermes-adapter] Streaming via LM Studio: ${resolvedId}`);
      return lmStudioStreamOneTurn(messages, resolvedId, tools, onTextDelta, abortCheck);
    default:
      return streamOneTurn(messages, model, tools, onTextDelta, abortCheck);
  }
}
...
async function execSpawnAgent(args) {
  ...
  const model = typeof args.model === "string" && args.model.trim() ? args.model.trim() : `lmstudio/${LM_STUDIO_DEFAULT_MODEL}`;
  const toolTier = typeof args.toolTier === "string" && ["none", "basic", "full"].includes(args.toolTier) ? args.toolTier : "none";
  ...
  if (toolTier !== "none") systemPrompt += `\n\nYou have ${toolTier} tool access. Use your tools to complete tasks — do not just describe what you would do.`;

  agentRegistry.set(newId, {
    id: newId, name, workspace: `${HOME}/.hermes/workspace-${slug}`,
    role, systemPrompt, settings: { wipe, continuity, model, boundaries, toolTier },
  });
...
async function execDelegateTask(args) {
  ...
  let responseText = "";
  try {
    // Resolve sub-agent tool tier
    const agentTools = resolveToolsForTier(agent.settings.toolTier);

    if (agentTools.length > 0) {
      // Sub-agent has tools — run through agentic loop (max 5 rounds)
      responseText = await runSubAgentAgenticLoop({
        sessionKey, agent, userMessage: message, model,
        tools: agentTools, emitDelta: (partial) => {
          responseText = partial;
          emitSub("delta", { message: { role: "assistant", content: partial } });
        },
      });
    } else {
      // No tools — simple streaming call
      const result = await routedStreamOneTurn(messages, model, [], (partial) => {
        responseText = partial;
        emitSub("delta", { message: { role: "assistant", content: partial } });
      }, null);
...
// ---------------------------------------------------------------------------
// Sub-agent tool definitions (basic + full tiers)
// ---------------------------------------------------------------------------

const { execSync } = require("child_process");

// Workspace root — resolved once at startup. Agents can only access files under this.
const WORKSPACE_ROOT = path.resolve(process.cwd());

/** Validate that a path is under the workspace root (prevents path traversal). */
...
// --- BASIC tools (read-only, safe) ---

const BASIC_AGENT_TOOLS = [
  {
    type: "function",
    function: {
      name: "read_file",
      description: "Read the contents of a file. Returns file text with optional line range.",
      parameters: {
        type: "object", required: ["path"],
        properties: {
          path: { type: "string", description: "Relative or absolute file path within the workspace." },
          startLine: { type: "number", description: "Start line (1-based). Omit to read from beginning." },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          directory: { type: "string", description: "Directory to search from. Default: workspace root." },
        },
      },
    },
  },
  {
    ...
    function: {
      name: "grep",
      description: "Search for text in files. Returns matching lines with file path and line number.",
      parameters: {
  ...
  },
  {
    ...
    function: {
      name: "web_fetch",
      description: "Fetch the text content of a URL via HTTP GET. Max 100KB response.",
      parameters: {
...
// --- FULL tools (basic + write/exec) ---

const FULL_ONLY_AGENT_TOOLS = [
  {
    type: "function",
    function: {
      name: "write_file",
      description: "Write content to a file (creates or overwrites). The directory is created if needed.",
      parameters: {
  ...
  },
  {
    ...
    function: {
      name: "patch_file",
      description: "Find and replace text in a file. The old text must appear exactly once.",
      parameters: {
        type: "object", required: ["path", "oldText", "newText"],
        properties: {
          path: { type: "string", description: "File path within workspace." },
          oldText: { type: "string", description: "Exact text to find (must appear once)." },
  ...
  },
  {
    ...
    function: {
      name: "run_command",
      description: "Execute a shell command and return stdout+stderr. 30s timeout. Workspace-scoped.",
      parameters: {
  ...
  },
  {
    ...
    function: {
      name: "git_status",
      description: "Run git status and return the porcelain output.",
      parameters: { type: "object", properties: {} },
    },
  },
  {
    ...
    function: {
      name: "git_diff",
      description: "Run git diff and return the output.",
      parameters: {
        type: "object",
        properties: {
          path: { type: "string", description: "Optional file path to diff. Omit for all changes." },
        },
      },
    },
  },
...
const FULL_AGENT_TOOLS = [...BASIC_AGENT_TOOLS, ...FULL_ONLY_AGENT_TOOLS];

function resolveToolsForTier(toolTier) {
  switch (toolTier) {
    case "basic": return BASIC_AGENT_TOOLS;
    case "full":  return FULL_AGENT_TOOLS;
    default:      return [];
  }
}

// ---------------------------------------------------------------------------
// Sub-agent tool executors
// ---------------------------------------------------------------------------
...
function execAgentReadFile(args) {
  const filePath = validateWorkspacePath(args.path);
  if (!fs.existsSync(filePath)) return JSON.stringify({ ok: false, error: `File not found: ${args.path}` });
  const content = fs.readFileSync(filePath, "utf8");
  const lines = content.split("\n");
  const start = Math.max(0, (typeof args.startLine === "number" ? args.startLine - 1 : 0));
  const end = typeof args.endLine === "number" ? Math.min(lines.length, args.endLine) : lines.length;
  return JSON.stringify({ ok: true, content: lines.slice(start, end).join("\n"), totalLines: lines.length });
}

function execAgentListDirectory(args) {
  const dirPath = validateWorkspacePath(args.path);
  if (!fs.existsSync(dirPath)) return JSON.stringify({ ok: false, error: `Directory not found: ${args.path}` });
  const entries = fs.readdirSync(dirPath, { withFileTypes: true });
  const items = entries.map((e) => e.isDirectory() ? `${e.name}/` : e.name);
  return JSON.stringify({ ok: true, items });
}

function execAgentSearchFiles(args) {
  const baseDir = validateWorkspacePath(args.directory || ".");
  ...
  function walk(dir, depth) {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        ...
        try { if (new RegExp(`^${simplePattern}$`, "i").test(e.name) || new RegExp(simplePattern, "i").test(relative)) results.push(relative); } catch { results.push(relative); }
      }
    }
  }
  walk(baseDir, 0);
  return JSON.stringify({ ok: true, matches: results.slice(0, MAX_RESULTS) });
}

function execAgentGrep(args) {
  const searchPath = validateWorkspacePath(args.path || ".");
  ...
  function searchFile(filePath) {
    ...
    for (let i = 0; i < lines.length && results.length < MAX_MATCHES; i++) {
      const match = isRegex ? new RegExp(query, "i").test(lines[i]) : lines[i].toLowerCase().includes(query.toLowerCase());
      if (match) results.push({ file: path.relative(WORKSPACE_ROOT, filePath).replace(/\\/g, "/"), line: i + 1, text: lines[i].slice(0, 200) });
    }
  }

  function walk(dir, depth) {
    if (results.length >= MAX_MATCHES || depth > 8) return;
    let entries;
    try { entries = fs.readdirSync(dir, { withFileTypes: true }); } catch { return; }
    for (const e of entries) {
  ...
  else if (stat?.isDirectory()) { walk(searchPath, 0); }
  else { return JSON.stringify({ ok: false, error: `Path not found: ${args.path}` }); }

  return JSON.stringify({ ok: true, matches: results });
}

function execAgentWebFetch(args) {
  const url = typeof args.url === "string" ? args.url.trim() : "";
  if (!url) return JSON.stringify({ ok: false, error: "URL required" });
  return new Promise((resolve) => {
    let parsedUrl;
    try { parsedUrl = new URL(url); } catch { resolve(JSON.stringify({ ok: false, error: "Invalid URL" })); return; }
    ...
    const req = transport.get(url, { timeout: 10_000 }, (res) => {
      ...
      res.on("data", (chunk) => {
        size += chunk.length;
        if (size > 102_400) { res.destroy(); resolve(JSON.stringify({ ok: false, error: "Response too large (>100KB)" })); return; }
        chunks.push(chunk);
      });
      res.on("end", () => resolve(JSON.stringify({ ok: true, status: res.statusCode, body: Buffer.concat(chunks).toString("utf8") })));
      res.on("error", (err) => resolve(JSON.stringify({ ok: false, error: err.message })));
    });
    req.on("error", (err) => resolve(JSON.stringify({ ok: false, error: err.message })));
    req.on("timeout", () => { req.destroy(); resolve(JSON.stringify({ ok: false, error: "Request timed out (10s)" })); });
  });
}

function execAgentWriteFile(args) {
  const filePath = validateWorkspacePath(args.path);
  const content = typeof args.content === "string" ? args.content : "";
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  fs.writeFileSync(filePath, content, "utf8");
  return JSON.stringify({ ok: true, path: path.relative(WORKSPACE_ROOT, filePath).replace(/\\/g, "/"), bytes: Buffer.byteLength(content) });
}

function execAgentPatchFile(args) {
  const filePath = validateWorkspacePath(args.path);
  if (!fs.existsSync(filePath)) return JSON.stringify({ ok: false, error: `File not found: ${args.path}` });
  const content = fs.readFileSync(filePath, "utf8");
  ...
  if (count === 0) return JSON.stringify({ ok: false, error: "oldText not found in file" });
  if (count > 1) return JSON.stringify({ ok: false, error: `oldText found ${count} times — must appear exactly once` });
  fs.writeFileSync(filePath, content.replace(oldText, newText), "utf8");
  return JSON.stringify({ ok: true, path: path.relative(WORKSPACE_ROOT, filePath).replace(/\\/g, "/") });
}

function execAgentRunCommand(args) {
  const command = typeof args.command === "string" ? args.command.trim() : "";
  if (!command) return JSON.stringify({ ok: false, error: "Command required" });
  for (const pattern of BLOCKED_COMMANDS) {
    if (pattern.test(command)) return JSON.stringify({ ok: false, error: `Blocked dangerous command pattern: ${pattern}` });
  }
  const cwd = args.cwd ? validateWorkspacePath(args.cwd) : WORKSPACE_ROOT;
  try {
    const output = execSync(command, { cwd, timeout: 30_000, encoding: "utf8", maxBuffer: 512 * 1024, windowsHide: true });
    return JSON.stringify({ ok: true, output: output.slice(0, 50_000) });
  } catch (err) {
    return JSON.stringify({ ok: false, exitCode: err.status, output: (err.stdout || "").slice(0, 20_000), stderr: (err.stderr || "").slice(0, 20_000) });
  }
}

function execAgentGitStatus() {
  try {
    const output = execSync("git status --porcelain", { cwd: WORKSPACE_ROOT, timeout: 10_000, encoding: "utf8", windowsHide: true });
    return JSON.stringify({ ok: true, output: output.slice(0, 20_000) });
  } catch (err) {
    return JSON.stringify({ ok: false, error: err.message });
  }
}

function execAgentGitDiff(args) {
  const filePath = args?.path ? validateWorkspacePath(args.path) : "";
  const cmd = filePath ? `git diff -- "${filePath}"` : "git diff";
  try {
    const output = execSync(cmd, { cwd: WORKSPACE_ROOT, timeout: 15_000, encoding: "utf8", windowsHide: true });
    return JSON.stringify({ ok: true, output: output.slice(0, 50_000) });
  } catch (err) {
    return JSON.stringify({ ok: false, error: err.message });
  }
...
async function executeAgentToolCall(tc) {
  console.log(`[hermes-adapter] Agent tool: ${tc.name}`, JSON.stringify(tc.args).slice(0, 120));
  switch (tc.name) {
    case "read_file":       return execAgentReadFile(tc.args);
    case "list_directory":  return execAgentListDirectory(tc.args);
    case "search_files":    return execAgentSearchFiles(tc.args);
    case "grep":            return execAgentGrep(tc.args);
    case "web_fetch":       return execAgentWebFetch(tc.args);
    case "write_file":      return execAgentWriteFile(tc.args);
    case "patch_file":      return execAgentPatchFile(tc.args);
    case "run_command":     return execAgentRunCommand(tc.args);
    case "git_status":      return execAgentGitStatus();
    case "git_diff":        return execAgentGitDiff(tc.args);
    default:                return JSON.stringify({ ok: false, error: `Unknown tool: ${tc.name}` });
  }
}

// ---------------------------------------------------------------------------
// Unified tool call executor — dispatches to team tools or agent tools
// ---------------------------------------------------------------------------
...
async function runAgenticLoop({ sessionKey, agentId, userMessage, model, tools, emitDelta, abortCheck, sendEvent }) {
  ...
  while (round < MAX_TOOL_ROUNDS) {
    round++;
    const { textContent, toolCalls, finishReason } = await routedStreamOneTurn(
...
async function handleMethod(method, params, id, sendEvent) {
  ...
  switch (method) {
    ...
    case "models.list":
      try {
        const allModels = [];
        // Anthropic models (if key configured)
        if (ANTHROPIC_API_KEY) {
          allModels.push({ id: `anthropic/${ANTHROPIC_MODEL}`, name: `Anthropic ${ANTHROPIC_MODEL}` });
        }
        // LM Studio models
        try {
          const lmModels = await fetchLmStudioModels();
          for (const m of lmModels) allModels.push({ id: `lmstudio/${m}`, name: `LM Studio ${m}` });
        } catch {}
        // Hermes models
        try {
          const hermesModels = await fetchHermesModels();
          for (const m of hermesModels) allModels.push({ id: m, name: m });
        } catch {}
        if (allModels.length === 0) allModels.push({ id: HERMES_MODEL, name: HERMES_MODEL });
        return resOk(id, { models: allModels });
      } catch {
...
function startAdapter() {
  ...
  httpServer.listen(ADAPTER_PORT, "127.0.0.1", () => {
    ...
    console.log(`[hermes-adapter] ✓ Default model: ${HERMES_MODEL}`);
    console.log(`[hermes-adapter] ✓ Multi-agent orchestration: ENABLED`);
    if (ANTHROPIC_API_KEY) {
      console.log(`[hermes-adapter] ✓ Anthropic backend: ${ANTHROPIC_API_URL} (model: ${ANTHROPIC_MODEL})`);
    } else {
      console.log(`[hermes-adapter]   Anthropic backend: NOT CONFIGURED (set ANTHROPIC_API_KEY)`);
    }
    console.log(`[hermes-adapter] ✓ LM Studio backend: ${LM_STUDIO_API_URL} (default: ${LM_STUDIO_DEFAULT_MODEL})`);
    console.log(`[hermes-adapter] ✓ Hermes backend: ${HERMES_API_URL}`);
