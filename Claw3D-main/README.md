...
If a local OpenClaw gateway is already using `18789`, the demo adapter now falls back to `ws://localhost:18890` and prints the exact URL in the terminal.
...
- `DEMO_ADAPTER_PORT` can pin the demo adapter to a specific local port. If it is unset and `18789` is already taken, the demo adapter falls back to `18890` automatically.
