...
export const GatewayConnectScreen = ({
  ...
  const [browserResetStatus, setBrowserResetStatus] = useState<"idle" | "resetting" | "done">("idle");
  const tokenOptional =
  ...
  const resetAdapterSettings = async () => {
    setResetStatus("resetting");
    try {
      const res = await fetch("/api/studio/reset-adapter", { method: "POST" });
      if (res.ok) {
        setResetStatus("done");
        window.setTimeout(() => setResetStatus("idle"), 2000);
        onConnect();
      } else {
        setResetStatus("error");
        window.setTimeout(() => setResetStatus("idle"), 3000);
      }
    } catch {
      setResetStatus("error");
      window.setTimeout(() => setResetStatus("idle"), 3000);
  ...
  const resetBrowserSession = () => {
    setBrowserResetStatus("resetting");
    onResetBrowserSession();
    setBrowserResetStatus("done");
    window.setTimeout(() => {
  ...
  return (
    <div className="mx-auto flex min-h-0 w-full max-w-[820px] flex-1 flex-col gap-5 overflow-y-auto pr-1">
      ...
      <div className="ui-card px-4 py-5 sm:px-6">
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <button
              type="button"
              className="ui-btn-secondary px-3 py-1.5 text-[11px] font-semibold tracking-[0.05em] text-red-400 hover:text-red-300"
              onClick={resetAdapterSettings}
              disabled={resetStatus === "resetting"}
              title="Clear saved gateway settings and re-detect adapter"
            >
            ...
            <button
              ...
              disabled={browserResetStatus === "resetting"}
              title="Clear browser-side gateway keys and reconnect cleanly"
            >
              {browserResetStatus === "resetting"
                ? "Clearing browser session…"
                : browserResetStatus === "done"
      ...
      <div className="ui-card px-4 py-4 sm:px-6 sm:py-5">
        <div className="space-y-1.5">
          ...
          <p className="text-xs leading-snug text-muted-foreground">
            You do not need this for the normal saved connection flow unless the gateway or adapter is
        ...
        <div className="mt-3 space-y-3">
          ...
          <div className="rounded-md border border-border bg-muted/30 px-3 py-3">
            ...
            <p className="mt-1 text-xs leading-snug text-muted-foreground">
              ...
              <span className="font-mono text-foreground"> Hermes backend</span> only if your Hermes adapter is
              running on this computer. The default local URL is
