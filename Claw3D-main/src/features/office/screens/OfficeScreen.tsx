...
import {
  ...
  type StudioAgentArchetype,
  type StudioCrewDoctrine,
  type StudioModelLoadout,
...
import { VideoPipelinePanel } from "@/features/office/components/panels/VideoPipelinePanel";
import type { PipelineAgentStatus } from "@/features/office/components/panels/VideoPipelinePanel";
...
const CREW_ARCHETYPE_LABELS: Record<StudioAgentArchetype, string> = {
...
const CREW_ARCHETYPE_ORDER: Record<StudioCrewDoctrine, StudioAgentArchetype[]> = {
  balanced: ["strategist", "builder", "scout", "lorekeeper"],
  specialized: ["builder", "strategist", "scout", "lorekeeper"],
  chaotic: ["scout", "lorekeeper", "builder", "strategist"],
};
...
const simplifyModelKey = (value: string) => {
  const trimmed = value.trim();
...
const resolveAgentArchetype = (
  agent: Pick<AgentState, "agentId" | "name" | "role">,
  doctrine: StudioCrewDoctrine
): StudioAgentArchetype => {
  const haystack = `${agent.name ?? ""} ${agent.role ?? ""}`.toLowerCase();
  if (/(strateg|plan|lead|architect|manager|product|coord)/.test(haystack)) {
    return "strategist";
  }
  if (/(build|code|dev|engineer|implement|tool)/.test(haystack)) {
    return "builder";
  }
  if (/(scout|research|review|test|qa|ops|observe|monitor)/.test(haystack)) {
    return "scout";
  }
  if (/(lore|write|docs|story|brand|prompt|content)/.test(haystack)) {
    return "lorekeeper";
  }
  const rotation = CREW_ARCHETYPE_ORDER[doctrine];
  return rotation[hashString(agent.agentId) % rotation.length] ?? "builder";
};

const resolveArchetypeModel = (
  loadout: StudioModelLoadout,
  archetype: StudioAgentArchetype
) => {
  if (archetype === "strategist" && loadout.strategistModel.trim()) {
    return loadout.strategistModel.trim();
  }
  if (archetype === "builder" && loadout.builderModel.trim()) {
    return loadout.builderModel.trim();
  }
  if (archetype === "scout" && loadout.scoutModel.trim()) {
    return loadout.scoutModel.trim();
  }
  if (archetype === "lorekeeper" && loadout.lorekeeperModel.trim()) {
    return loadout.lorekeeperModel.trim();
  }
  return loadout.primaryModel.trim();
};

const buildOfficeAgentSubtitle = (agent: AgentState, loadout: StudioModelLoadout) => {
  const archetype = resolveAgentArchetype(agent, loadout.doctrine);
  const modelLabel = simplifyModelKey(resolveArchetypeModel(loadout, archetype));
  const parts = [CREW_ARCHETYPE_LABELS[archetype]];
  if (modelLabel) {
    parts.push(modelLabel);
  } else if (loadout.fallbackModel.trim()) {
    parts.push(`fallback ${simplifyModelKey(loadout.fallbackModel)}`);
  }
  if (agent.role?.trim()) {
    parts.push(agent.role.trim());
  }
  return parts.join(" // ");
};

const mapAgentToOffice = (agent: AgentState, loadout: StudioModelLoadout): OfficeAgent => {
  const subtitle = buildOfficeAgentSubtitle(agent, loadout);
...
export function OfficeScreen({
  ...
  const runtimeSupportsFiles = supportsCapability("files");
  ...
  // Video pipeline agent status for the Pipeline tab.
  // These map to the 3 video agents registered in the demo gateway.
  const videoPipelineAgents = useMemo(() => {
    const VIDEO_AGENT_IDS: Record<string, { role: string; idleLine: string }> = {
      "video-trend-scout": {
        role: "Trend Research",
        idleLine: "Monitoring TikTok trending feeds. Say \"find viral trends\" to start.",
      },
      "video-content-forge": {
        role: "Content Generation",
        idleLine: "ComfyUI + FFmpeg pipeline ready. Say \"generate video\" to create content.",
      },
      "video-tiktok-publisher": {
        role: "TikTok Publisher",
        idleLine: "Publishing desk ready. Say \"upload to tiktok\" after video is generated.",
      },
    };
    return Object.entries(VIDEO_AGENT_IDS).map(([agentId, info]) => {
      const agent = state.agents.find((a) => a.agentId === agentId);
      const agentStatus: PipelineAgentStatus =
        agent?.status === "running" ? "working" : "idle";
      return {
        name: agent?.name || agentId.replace("video-", "").replace(/-/g, " "),
        role: info.role,
        status: agentStatus,
        statusLine: agent?.status === "running"
          ? (agent.streamText?.slice(0, 120) || "Processing…")
          : info.idleLine,
      };
    });
  }, [state.agents]);
  ...
  const loadoutSignature = useMemo(
    () =>
      JSON.stringify({
        doctrine: modelLoadout.doctrine,
        primaryModel: modelLoadout.primaryModel,
        fallbackModel: modelLoadout.fallbackModel,
        strategistModel: modelLoadout.strategistModel,
        builderModel: modelLoadout.builderModel,
        scoutModel: modelLoadout.scoutModel,
        lorekeeperModel: modelLoadout.lorekeeperModel,
      }),
    [modelLoadout]
  );
  const crewResonancePct = useMemo(() => {
    if (state.agents.length === 0) return 0;
    const activeArchetypes = new Set(
      state.agents.map((agent) => resolveAgentArchetype(agent, modelLoadout.doctrine))
    );
    const routeCount = [
      modelLoadout.primaryModel,
      modelLoadout.fallbackModel,
      modelLoadout.strategistModel,
      modelLoadout.builderModel,
      modelLoadout.scoutModel,
      modelLoadout.lorekeeperModel,
    ].filter((value) => value.trim().length > 0).length;
    const coverageScore = activeArchetypes.size / 4;
    const routingScore = routeCount / 6;
    return Math.round((coverageScore * 0.65 + routingScore * 0.35) * 100);
  }, [modelLoadout, state.agents]);
  ...
  const openAgentEditor = useCallback(
    async (agentId: string, initialSection: AgentEditorSection = "avatar") => {
      if (initialSection !== "avatar" && !runtimeSupportsFiles) {
        const ready = await ensureGatewayBackedConnection();
        if (!ready) {
          setError(
            "Agent file editing requires a gateway-backed adapter. Connect to Hermes, Demo, or OpenClaw and try again."
          );
          return;
        }
      }
    ...
    [ensureGatewayBackedConnection, focusLocalAgent, runtimeSupportsFiles, setError],
  );
  ...
  const officeAgents = useMemo(() => {
    ...
    const nextOfficeAgents = state.agents.map((agent) => {
      ...
      if (
        ...
        cached.loadoutSignature === loadoutSignature
      ) {
      ...
      const effectiveAgent: AgentState =
        latchedWorking && agent.status !== "error"
          ...
          : (deskHeld || gymHeld || qaHeld || phoneBoothHeld || smsBoothHeld) &&
            ...
            : agent;
      const officeAgent = mapAgentToOffice(effectiveAgent, modelLoadout);
  ...
  return (
    <main className="relative h-full w-full overflow-hidden bg-black">
      ...
      {showGatewayConnectOverlay ? (
        <div className="pointer-events-auto absolute inset-0 z-50 overflow-y-auto bg-[#120a05]/76 px-4 py-6 sm:py-10">
          <div className="mx-auto w-full max-w-[860px] max-h-[calc(100vh-3rem)] rounded-2xl border border-amber-900/55 bg-[#120a05]/98 p-3 shadow-2xl sm:max-h-[calc(100vh-5rem)] overflow-y-auto">
      ...
      <section className="relative h-full min-h-0 min-w-0 overflow-hidden">
        <RetroOffice3D
          ...
          selectedAdapterType={selectedAdapterType}
          activeAdapterType={activeAdapterType}
          modelCatalog={gatewayModels}
          crewDoctrine={crewDoctrine}
          primaryModel={primaryModel}
          fallbackModel={fallbackModel}
          strategistModel={strategistModel}
          builderModel={builderModel}
          scoutModel={scoutModel}
          lorekeeperModel={lorekeeperModel}
          crewResonancePct={crewResonancePct}
          ...
          onCrewDoctrineChange={setCrewDoctrine}
          onPrimaryModelChange={setPrimaryModel}
          onFallbackModelChange={setFallbackModel}
          onArchetypeModelChange={setArchetypeModel}
      ...
      {agentEditorAgent ? (
        <AgentEditorModal
          ...
          open
          client={status === "connected" && runtimeSupportsFiles ? client : null}
          brainFilesUnavailableReason={
            status === "connected" && !runtimeSupportsFiles
              ? `The active ${activeAdapterType} runtime does not expose agent file APIs. Connect to Hermes, Demo, or OpenClaw to edit brain files.`
              : null
          }
          agents={state.agents}
