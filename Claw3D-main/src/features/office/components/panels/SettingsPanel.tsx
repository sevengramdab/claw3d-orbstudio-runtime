...
const DOCTRINE_OPTIONS: Array<{ value: StudioCrewDoctrine; label: string; description: string }> = [
  {
    value: "balanced",
    label: "Balanced crew",
    description: "Spread the office across planning, building, scouting, and lore roles.",
  },
  {
    value: "specialized",
    label: "Specialized crew",
    description: "Bias the office toward execution-heavy builder routing.",
  },
  {
    value: "chaotic",
    label: "Chaotic crew",
    description: "Rotate archetypes more aggressively for experimentation and discovery.",
  },
];

const ARCHETYPE_FIELDS: Array<{ archetype: StudioAgentArchetype; label: string }> = [
  { archetype: "strategist", label: "Strategist" },
  { archetype: "builder", label: "Builder" },
  { archetype: "scout", label: "Scout" },
  { archetype: "lorekeeper", label: "Lorekeeper" },
];

const formatModelLabel = (model: GatewayModelChoice) => {
...
type SettingsPanelProps = {
  ...
  crewDoctrine: StudioCrewDoctrine;
  primaryModel: string;
  fallbackModel: string;
  strategistModel: string;
  builderModel: string;
  scoutModel: string;
  lorekeeperModel: string;
  onCrewDoctrineChange: (value: StudioCrewDoctrine) => void;
  onPrimaryModelChange: (value: string) => void;
  onFallbackModelChange: (value: string) => void;
  onArchetypeModelChange: (archetype: StudioAgentArchetype, value: string) => void;
  officeTitle: string;
...
export function SettingsPanel({
  ...
  scoutModel,
  lorekeeperModel,
  onCrewDoctrineChange,
  onPrimaryModelChange,
  onFallbackModelChange,
  onArchetypeModelChange,
  officeTitle,
  officeTitleLoaded,
  onOfficeTitleChange,
  remoteOfficeEnabled,
  ...
  const modelOptions = useMemo(() => {
    ...
    for (const model of modelCatalog) {
      ...
      map.set(value, { value, label: formatModelLabel(model) });
    }
  ...
  const archetypeModels: Record<StudioAgentArchetype, string> = {
    strategist: strategistModel,
    builder: builderModel,
    scout: scoutModel,
    lorekeeper: lorekeeperModel,
  };
  ...
  return (
    <div className="px-4 py-4">
      ...
      <div className="mt-3 rounded-lg border border-cyan-500/10 bg-black/20 px-4 py-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-[11px] font-medium text-white">Model loadout</div>
            <div className="mt-1 text-[10px] text-white/75">
              Route the office through a default runtime pair, then bias each archetype toward a specific model.
            </div>
          </div>
          <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-cyan-200/70">
            {modelOptions.length > 0 ? `${modelOptions.length} models` : "Manual"}
          </span>
        </div>
        <div className="mt-3 grid gap-3">
          <div>
            ...
            <select
              value={crewDoctrine}
              onChange={(event) => onCrewDoctrineChange(event.target.value as StudioCrewDoctrine)}
            ...
            <div className="mt-1 text-[10px] text-white/50">
              {DOCTRINE_OPTIONS.find((option) => option.value === crewDoctrine)?.description}
            </div>
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-md border border-cyan-500/10 bg-black/25 px-3 py-2 text-[11px] text-cyan-100 outline-none transition-colors focus:border-cyan-400/30"
              >
                <option value="">No fallback override</option>
                {modelOptions.map((option) => (
                  <option key={option.value} value={option.value}>
                    {option.label}
                  </option>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {ARCHETYPE_FIELDS.map(({ archetype, label }) => (
              <div key={archetype}>
                <div className="mb-1 text-[10px] uppercase tracking-[0.14em] text-cyan-100/65">
                  {label} route
                </div>
                <select
                  value={archetypeModels[archetype]}
                  onChange={(event) => onArchetypeModelChange(archetype, event.target.value)}
                  className="w-full rounded-md border border-cyan-500/10 bg-black/25 px-3 py-2 text-[11px] text-cyan-100 outline-none transition-colors focus:border-cyan-400/30"
                >
                  <option value="">Follow primary runtime</option>
                  {modelOptions.map((option) => (
                    <option key={`${archetype}-${option.value}`} value={option.value}>
                      {option.label}
                    </option>
                  ))}
                </select>
          ...
          <div className="rounded-md border border-cyan-500/10 bg-black/15 px-3 py-3 text-[10px] leading-5 text-white/60">
            Agents in the 3D office get a stable archetype tag and inherit the matching routed model. Leave an archetype blank to fall back to the primary runtime.
          </div>
        </div>
      </div>
      <div className="mt-3 rounded-lg border border-cyan-500/10 bg-black/20 px-4 py-3">
