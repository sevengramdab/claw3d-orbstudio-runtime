...
import { deriveSkillReadinessState, resolvePreferredInstallOption } from "@/lib/skills/presentation";
...
const TASK_MANAGER_SKILL_KEY = "task-manager";
...
const findSkillByKey = (skills: SkillStatusEntry[], skillKey: string): SkillStatusEntry | null => {
  const normalizedSkillKey = skillKey.trim().toLowerCase();
  return (
    skills.find((skill) => {
      const nextSkillKey = skill.skillKey.trim().toLowerCase();
      const nextName = skill.name.trim().toLowerCase();
      return nextSkillKey === normalizedSkillKey || nextName === normalizedSkillKey;
    }) ?? null
  );
};
...
export const useOfficeSkillsMarketplace = ({
  ...
  const taskManagerBootstrapAttemptsRef = useRef<Set<string>>(new Set());
  ...
  const ensurePackagedSkillEnabled = useCallback(
    async (params: {
      skillKey: string;
      agentId: string;
      ...
      if (!packagedSkill) {
        throw new Error(
          `No packaged marketplace skill was found for ${params.skillKey.trim() || "that entry"}.`
        );
      }

      const targetAgentId = params.agentId.trim();
      if (!targetAgentId) {
        throw new Error("Select an agent before installing marketplace skills.");
      }
      ...
      try {
        ...
        const installedSkill = findSkillByKey(initialReport.skills, packagedSkill.skillKey);

        if (!installedSkill) {
          params.onProgress?.({
            percent: 38,
            message: `Installing ${packagedSkill.skillKey} into the workspace.`,
          });
        ...
        params.onProgress?.({
          percent: 62,
          message: `Enabling ${packagedSkill.skillKey} for this gateway.`,
        });
        await updateSkill(client, { skillKey: packagedSkill.skillKey, enabled: true });

        params.onProgress?.({
          percent: 78,
          message: `Enabling ${packagedSkill.skillKey} for the selected agent.`,
        });
        ...
        params.onProgress?.({
          percent: 100,
          message: `${packagedSkill.skillKey} installed and enabled.`,
        });

        if (!params.suppressSuccessMessage) {
          const agentName =
            agents.find((agent) => agent.agentId === targetAgentId)?.name ?? "the selected agent";
          setMessage({
            kind: "success",
            text:
              params.successMessage ??
              `Installed and enabled ${packagedSkill.name.trim()} for ${agentName}.`,
          });
        }
      } catch (err) {
        const nextMessage =
          err instanceof Error ? err.message : "Failed to install and enable the skill.";
        setError(nextMessage);
        setMessage({
          kind: "error",
          text: nextMessage,
    ...
    [agents, client, loadMarketplace, onSkillActivityEnd, onSkillActivityStart],
  );

  useEffect(() => {
    if (!enabled || status !== "connected" || loading || busySkillKey) {
      return;
    }
    const targetAgentId = selectedAgentId?.trim() ?? "";
    if (!targetAgentId || !skillsReport) {
      return;
    }

    const workspaceDir = skillsReport.workspaceDir?.trim() ?? "";
    if (!workspaceDir || isLikelyRootWorkspace(workspaceDir)) {
      return;
    }

    const taskManagerSkill = findSkillByKey(skillsReport.skills, TASK_MANAGER_SKILL_KEY);
    if (taskManagerSkill && deriveSkillReadinessState(taskManagerSkill) === "ready") {
      return;
    }

    const bootstrapKey = `${workspaceDir.toLowerCase()}::${targetAgentId.toLowerCase()}`;
    if (taskManagerBootstrapAttemptsRef.current.has(bootstrapKey)) {
      return;
    }
    taskManagerBootstrapAttemptsRef.current.add(bootstrapKey);

    void ensurePackagedSkillEnabled({
      skillKey: TASK_MANAGER_SKILL_KEY,
      agentId: targetAgentId,
      suppressSuccessMessage: true,
    }).catch(() => {
      // Keep background bootstrap best-effort; manual install remains available.
    });
  }, [busySkillKey, enabled, ensurePackagedSkillEnabled, loading, selectedAgentId, skillsReport, status]);
  ...
  const handleInstallPackagedSkillAndEnable = useCallback(
    async (params: {
      ...
      await ensurePackagedSkillEnabled({
        skillKey: packagedSkill.skillKey,
        agentId: targetAgentId,
