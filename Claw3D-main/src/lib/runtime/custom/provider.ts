...
const normalizeOpenAiCompatibleModelChoices = (
  payload: OpenAiCompatibleModelsResponse | null
): string[] => {
...
export class CustomRuntimeProvider implements RuntimeProvider {
  ...
  async fetchHealth(): Promise<CustomRuntimeHealthResponse> {
    try {
      return await this.fetchJson<CustomRuntimeHealthResponse>("/health");
    } catch {
      const modelChoices = await this.fetchOpenAiCompatibleModelChoices();
      return {
        ok: modelChoices.length > 0,
        status: modelChoices.length > 0 ? "ok" : "unavailable",
  ...
  async fetchState(): Promise<CustomRuntimeStateResponse> {
    try {
      return await this.fetchJson<CustomRuntimeStateResponse>("/state");
    } catch {
      const modelChoices = await this.fetchOpenAiCompatibleModelChoices();
      const activeModel = modelChoices[0] ?? null;
      return {
        identity: {
          name: "Assistant",
          role: "assistant",
  ...
  async fetchRegistry(): Promise<CustomRuntimeRegistryResponse> {
    try {
      return await this.fetchJson<CustomRuntimeRegistryResponse>("/registry");
    } catch {
      const modelChoices = await this.fetchOpenAiCompatibleModelChoices();
      return {
        models: Object.fromEntries(modelChoices.map((id) => [id, { id }])),
      };
    }
  }
  ...
  private async fetchOpenAiCompatibleModelChoices(): Promise<string[]> {
    const payload = await this.fetchJson<OpenAiCompatibleModelsResponse>("/v1/models");
    return normalizeOpenAiCompatibleModelChoices(payload);
  }
}
