...
const resolveDefaultGatewayProfile = (
  ...
  const profile = localDefaults?.profiles?.[adapterType];
  if (profile?.url.trim()) {
    return {
      url: profile.url,
      token: profile.token,
  ...
  switch (adapterType) {
    case "custom":
      return {
        url:
          localDefaults?.adapterType === "custom" && localDefaults.url.trim()
            ? localDefaults.url
            : DEFAULT_CUSTOM_RUNTIME_URL,
        token: localDefaults?.adapterType === "custom" ? localDefaults.token : "",
      };
    case "demo":
    case "hermes":
      return {
        url:
          localDefaults?.adapterType === adapterType && localDefaults.url.trim()
            ? localDefaults.url
            : "ws://localhost:18789",
        token: localDefaults?.adapterType === adapterType ? localDefaults.token : "",
      };
    case "openclaw":
    default:
      return {
        url:
          localDefaults?.adapterType === "openclaw" && localDefaults.url.trim()
            ? localDefaults.url
            : DEFAULT_UPSTREAM_GATEWAY_URL,
        token: localDefaults?.adapterType === "openclaw" ? localDefaults.token : "",
      };
  }
};

export const resolveGatewayBackedFallbackProfile = (
  profiles: Partial<Record<StudioGatewayAdapterType, { url: string; token: string }>>,
  localDefaults: StudioGatewaySettings | null
): { adapterType: Exclude<StudioGatewayAdapterType, "custom">; url: string; token: string } | null => {
  for (const adapterType of ["demo", "hermes", "openclaw"] as const) {
    const profile =
      profiles[adapterType] ??
      (adapterType === "openclaw" ? resolveDefaultGatewayProfile(adapterType, localDefaults) : null);
    if (!profile?.url.trim()) {
      continue;
    }
    return {
...
export type GatewayConnectionState = {
  ...
  ensureGatewayBackedConnection: () => Promise<boolean>;
  disconnect: () => void;
...
export const useGatewayConnection = (
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        ...
        const savedProfiles = gateway?.profiles
          ? normalizeGatewayProfilesPublic(gateway.profiles)
          : undefined;
        const savedAdapterType = normalizeGatewayAdapterType(gateway?.adapterType);
        ...
        const savedSelectedProfile = savedProfiles?.[nextAdapterType] ?? null;
        const resolvedUrl = hasSavedUrl
          ? gateway!.url
          : savedSelectedProfile?.url ||
            lastKnownGoodForSelectedAdapter?.url ||
            normalizedDefaults?.profiles?.[nextAdapterType]?.url ||
            (normalizedDefaults?.adapterType === nextAdapterType ? normalizedDefaults.url : "") ||
            DEFAULT_UPSTREAM_GATEWAY_URL;
        const baseProfiles = {
          ...(savedProfiles ?? {}),
  ...
  const connectGatewayBackedTarget = useCallback(
    async (
      adapterType: Exclude<StudioGatewayAdapterType, "custom">,
      ...
      try {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
          try {
            await client.connect({
              ...
              token: nextToken,
              authScopeKey: nextGatewayUrl,
              clientName: resolveGatewayClientName(adapterType, nextGatewayUrl),
              disableDeviceAuth: adapterType !== "openclaw",
            });
            lastError = null;
            break;
          } catch (err) {
        ...
        await ensureGatewayReloadModeHotForLocalStudio({
          client,
          upstreamGatewayUrl: nextGatewayUrl,
        });
        ...
        setHasLastKnownGoodState(nextDetectedAdapterType === adapterType);
  ...
  const connect = useCallback(async () => {
    ...
    let activeAdapterType = selectedAdapterType;
    let activeGatewayUrl = gatewayUrl;
    ...
    if (selectedAdapterType === "custom") {
      ...
      try {
        ...
        const fallback = resolveGatewayBackedFallbackProfile(
          adapterProfiles,
        ...
        activeAdapterType = fallback.adapterType;
        activeGatewayUrl = fallback.url;
        activeToken = fallback.token;
        setSelectedAdapterTypeState(fallback.adapterType);
        setGatewayUrl(fallback.url);
        setToken(fallback.token);
        setDetectedAdapterType(null);
        setHasLastKnownGoodState(true);
        gatewayDebugLog("connect:custom-fallback", {
          fallbackAdapterType: fallback.adapterType,
          fallbackGatewayUrl: fallback.url,
        });
      }
    }
    if (activeAdapterType === "custom") {
      return;
    }
    await connectGatewayBackedTarget(activeAdapterType, activeGatewayUrl, activeToken);
  }, [adapterProfiles, connectGatewayBackedTarget, gatewayUrl, localGatewayDefaults, selectedAdapterType, settingsCoordinator, token]);

  const ensureGatewayBackedConnection = useCallback(async () => {
    if (retryTimerRef.current) {
      clearTimeout(retryTimerRef.current);
      retryTimerRef.current = null;
    }
    ...
    const fallback =
      selectedAdapterType === "custom"
        ? resolveGatewayBackedFallbackProfile(adapterProfiles, localGatewayDefaults)
        : {
    ...
    if (!fallback) {
      setConnectErrorCode("studio.gateway_backed_adapter_required");
      setError(
    ...
    const currentActiveAdapterType =
      status === "connected" ? detectedAdapterType ?? selectedAdapterType : selectedAdapterType;

    if (
      status === "connected" &&
      currentActiveAdapterType === fallback.adapterType &&
      gatewayUrl.trim() === fallback.url.trim()
    ) {
      return true;
    }
    ...
    gatewayDebugLog("connect:ensure-gateway-backed", {
      previousAdapterType: selectedAdapterType,
      fallbackAdapterType: fallback.adapterType,
      fallbackGatewayUrl: fallback.url,
    });

    return await connectGatewayBackedTarget(
      fallback.adapterType,
      fallback.url,
      fallback.token
    );
  }, [adapterProfiles, client, connectGatewayBackedTarget, detectedAdapterType, gatewayUrl, localGatewayDefaults, selectedAdapterType, status, token]);
  ...
  const resetBrowserSession = useCallback(() => {
    gatewayDebugLog("reset-browser-session", { selectedAdapterType, status });
    setError(null);
    setConnectErrorCode(null);
    setDetectedAdapterType(null);
    retryAttemptRef.current = 0;
