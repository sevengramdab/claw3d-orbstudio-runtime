...
export type StudioAgentArchetype =
  | "strategist"
  | "builder"
  | "scout"
  | "lorekeeper";

export type StudioCrewDoctrine = "balanced" | "specialized" | "chaotic";

export type StudioModelLoadout = {
...
export type StudioModelLoadoutPatch = {
  doctrine?: StudioCrewDoctrine;
...
export type StudioOfficePreference = {
  ...
  modelLoadout: StudioModelLoadout;
};
...
export type StudioOfficePreferencePatch = {
  ...
  modelLoadout?: StudioModelLoadoutPatch | null;
};
...
const normalizeModelLoadoutModel = (value: unknown, fallback = "") => {
  if (value === null) return "";
  const model = coerceString(value);
  return (model || fallback).slice(0, 160);
};

const normalizeCrewDoctrine = (
  value: unknown,
  fallback: StudioCrewDoctrine = "balanced"
): StudioCrewDoctrine => {
  const doctrine = coerceString(value).toLowerCase();
  if (doctrine === "balanced" || doctrine === "specialized" || doctrine === "chaotic") {
    return doctrine;
  }
  return fallback;
};
...
export const defaultStudioModelLoadout = (): StudioModelLoadout => ({
  doctrine: "balanced",
  primaryModel: "",
  fallbackModel: "",
  strategistModel: "",
...
const normalizeModelLoadout = (
  value: unknown,
  fallback: StudioModelLoadout = defaultStudioModelLoadout()
): StudioModelLoadout => {
  if (!isRecord(value)) return fallback;
  return {
    doctrine: normalizeCrewDoctrine(value.doctrine, fallback.doctrine),
    primaryModel: normalizeModelLoadoutModel(value.primaryModel, fallback.primaryModel),
    fallbackModel: normalizeModelLoadoutModel(value.fallbackModel, fallback.fallbackModel),
    strategistModel: normalizeModelLoadoutModel(
      value.strategistModel,
      fallback.strategistModel
    ),
    builderModel: normalizeModelLoadoutModel(value.builderModel, fallback.builderModel),
    scoutModel: normalizeModelLoadoutModel(value.scoutModel, fallback.scoutModel),
    lorekeeperModel: normalizeModelLoadoutModel(
      value.lorekeeperModel,
      fallback.lorekeeperModel
    ),
  };
};
...
export const defaultStudioOfficePreference = (): StudioOfficePreference => ({
  ...
  companyRoleTitles: [],
  companyPlanJson: "",
  modelLoadout: defaultStudioModelLoadout(),
});
...
export const sanitizeStudioOfficePreference = (
  ...
  modelLoadout: value.modelLoadout,
});
...
const normalizeOfficePreference = (
  ...
  return {
    ...
    modelLoadout: normalizeModelLoadout(value.modelLoadout, fallback.modelLoadout),
  };
};
      ...
      nextOffice[gatewayKey] = normalizeOfficePreference(
        {
          ...
          ...officePatch,
          modelLoadout: {
            ...fallback.modelLoadout,
            ...(isRecord(officePatch.modelLoadout) ? officePatch.modelLoadout : {}),
          },
        },
        fallback
      );
