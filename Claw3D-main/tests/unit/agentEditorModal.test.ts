...
describe("AgentEditorModal", () => {
  ...
  it("shows the runtime capability message when brain files are unavailable", () => {
    const agent = buildAgent();
    ...
    render(
      createElement(AgentEditorModal, {
        ...
        brainFilesUnavailableReason:
          "The active custom runtime does not expose agent file APIs. Connect to Hermes, Demo, or OpenClaw to edit brain files.",
        agents: [agent],
        agent,
        initialSection: "IDENTITY.md",
        onClose: () => {},
        onAvatarSave: () => {},
      }),
    );
