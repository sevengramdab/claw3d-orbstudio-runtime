...
const setupAndImportHook = async (gatewayUrl: string | null) => {
  ...
  vi.doMock("../../src/lib/runtime/custom/http", () => ({
    probeCustomRuntime: vi.fn(async () => undefined),
  }));
...
describe("useGatewayConnection", () => {
  ...
  it("uses_a_small_initial_auto_connect_delay_for_hermes_demo_and_custom", async () => {
    const mod = await import("@/lib/gateway/GatewayClient");
    expect(mod.resolveInitialGatewayAutoConnectDelayMs("openclaw")).toBe(0);
    expect(mod.resolveInitialGatewayAutoConnectDelayMs("custom")).toBe(900);
  ...
  it("retries_the_initial_connect_for_hermes_demo_and_custom", async () => {
    const mod = await import("@/lib/gateway/GatewayClient");
    expect(mod.resolveInitialGatewayConnectAttemptCount("openclaw", false)).toBe(1);
    expect(mod.resolveInitialGatewayConnectAttemptCount("custom", false)).toBe(2);
    ...
    expect(mod.resolveInitialGatewayConnectAttemptCount("custom", true)).toBe(2);
  ...
  it("does_not_leak_a_custom_runtime_url_into_gateway_backed_fallback_profiles", async () => {
    const mod = await import("@/lib/gateway/GatewayClient");
    expect(
      mod.resolveGatewayBackedFallbackProfile({}, {
  ...
  it("does_not_prompt_to_reconnect_for_custom_with_last_known_good_state", async () => {
    const { useGatewayConnection } = await setupAndImportHook(null);
    const coordinator = {
      loadSettingsEnvelope: async () => ({
        settings: {
  ...
  it("persists_last_known_good_for_custom_after_a_successful_probe", async () => {
    const { useGatewayConnection } = await setupAndImportHook(null);
    const patches: unknown[] = [];
    const coordinator = {
      loadSettingsEnvelope: async () => ({
  ...
  it("uses_the_saved_adapter_type_even_when_only_adapter_profiles_are_persisted", async () => {
    const { useGatewayConnection } = await setupAndImportHook(null);
    const coordinator = {
      loadSettingsEnvelope: async () => ({
        settings: {
        ...
        localGatewayDefaults: {
          ...
          adapterType: "openclaw",
          profiles: {
            hermes: { url: "ws://localhost:19444", tokenConfigured: false },
    ...
    expect(screen.getByTestId("gatewayUrl")).toHaveTextContent("ws://localhost:19444");
  });
});
