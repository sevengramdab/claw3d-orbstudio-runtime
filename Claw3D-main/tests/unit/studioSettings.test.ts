...
describe("studio settings normalization", () => {
  ...
  it("normalizes office model loadout preferences per gateway", () => {
    const normalized = normalizeStudioSettings({
      office: {
        " ws://localhost:18789 ": {
          modelLoadout: {
            doctrine: " specialized ",
            primaryModel: " ollama/qwen2.5-coder:14b ",
            fallbackModel: " anthropic/claude-sonnet-4 ",
            strategistModel: " anthropic/claude-opus-4 ",
            builderModel: " ollama/qwen2.5-coder:14b ",
            scoutModel: " ollama/deepseek-r1:14b ",
            lorekeeperModel: " openai/gpt-4.1 ",
          },
        },
      },
    });
  ...
  it("merges office model loadout patches without dropping existing routes", () => {
    const current = normalizeStudioSettings({
      office: {
        "ws://localhost:18789": {
          modelLoadout: {
